...
class Metric(ABC):
    ...
    @abstractmethod
    def log(self, results, **kwargs):
        ...


class IrisMetric(ABC, Metric):
    @abstractmethod
    @property
    def columns(self):
        ...
    ...
    @abstractmethod
    def log(self, results, polar_image, polar_filtered, mask):
        ...


class GazeMetric(ABC, Metric):
    @abstractmethod
    @property
    def columns(self):
        ...
    ...
    @abstractmethod
    def log(self, results, model: GazeModel, sample: GazeImage, filtered: np.ndarray):
        ...


class PupilMetric(ABC, Metric):
    @abstractmethod
    @property
    def columns(self):
        ...
    ...
    @abstractmethod
    def log(self, results, pupil_sample: PupilSample, filtered: np.ndarray):
        ...
...
class GradientEntropy(IrisMetric):
    ...
    def log(self, results, polar_image, polar_filtered, mask):
        ...
        results['gradient_entropy_source'] = entropy_source
        results['gradient_entropy_filtered'] = entropy_filtered
        results['gradient_mutual_information'] = mutual_information
...
class GaborEntropy(IrisMetric):
    ...
    def log(self, results, polar_image, polar_filtered, mask):
        angles = np.linspace(0, np.pi, self.angles_per_scale)  # TODO: Consider subtracting small amount
        for scale in range(self.scales):
            for theta in angles:
                ...
                results[f'gabor_entropy_source_@{1 / scale}x'] = entropy_source
                results[f'gabor_entropy_filtered_@{1 / scale}x'] = entropy_filtered
                results[f'gabor_mutual_information_@{1 / scale}x'] = mutual_information

            polar_image = cv.pyrDown(polar_image)
            polar_filtered = cv.pyrDown(polar_filtered)
            mask = cv.resize(mask, polar_image.shape, interpolation=cv.INTER_NEAREST)
...
class GazeAccuracy(GazeMetric):
    ...
    def log(self, results, model: GazeModel, sample: GazeImage, filtered: np.ndarray):
        ...
        results['gaze_angle_error_source'] = angle_error_source
        results['gaze_angle_error_filtered'] = angle_error_filtered
...
class PupilDetectionError(PupilMetric):
    def __init__(self, detectors: List[PupilDetector]):
    ...
    def log(self, results, pupil_sample: PupilSample, filtered: np.ndarray):
        for d in self.detectors:
            ...
            results[f'pupil_distance_{d.name}_pixel_error_source'] = dist_unmodified
            results[f'pupil_distance_{d.name}_pixel_error_filtered'] = dist_filtered


class ImageSimilarity(IrisMetric):
    columns = ['iris_code_distance', 'image_norm_distance']

    def __init__(self, angles, angular_resolution, radial_resolution, ):
        self.encoder = SKImageIrisCodeEncoder()
    ...
    def log(self, results, polar_image, polar_filtered, mask):
        ...
        results['iris_code_similarity'] = iris_code_similarity

        source_masked = polar_image * mask
        source_masked /= np.linalg.norm(source_masked)
        filtered_masked = polar_filtered * mask
        norm = np.linalg.norm(filtered_masked)
        if norm == 0:
            return np.nan
        else:
            filtered_masked /= norm
            similarity = 1 - np.linalg.norm(source_masked - filtered_masked)
            results['image_normalized_similarity'] = similarity
